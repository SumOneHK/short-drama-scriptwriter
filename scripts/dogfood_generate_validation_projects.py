...
def cn_outline_entries() -> tuple[str, str]:
    ...
    for i in range(1, 61):
        ...
        details.append(
            ...
            f"- 本集卡点设计：IAA，5s 逼签/压迫，25s 证据露出，50s {ep['hook']}。[MUST]\n"
            f"- 结尾钩子：{ep['hook']}。[MUST]\n"
...
def us_outline_entries() -> tuple[str, str]:
    ...
    for i in range(1, 61):
        ...
        details.append(
            ...
            f"- 本集卡点设计：IAA，5s threat，25s proof，50s {ep['hook']}。[MUST]\n"
            f"- 结尾钩子：{ep['hook']}。[MUST]\n"
...
| QC-003 | {GENERATED_AT} | T15 | 全剧复核 | dogfood-simulated | EP001-EP060 | P2 | 个别场景复用偏高，制作友好但视觉变化不足 | 接受风险 | production-enhanced |
...
3. 交付前复核：{market}项目保留高复用场景策略，原因是 AI 漫剧 60 集制作预算优先稳一致性。
"""
...
- 制作边界：高复用城市空间，少大场面群像，重证据、对峙、公开场。
...
- 制作约束：少群像大场面，用近景、道具、灯光和文件推进。 [MUST]
...
""",
    ...
    write(project / "90-内部工作稿" / "质检检查点.md", common_qc(project_name, "抖音国内"))
...
- 目标观众：TikTok 欧美女性向 revenge / hidden-heir 观众。 [MUST]
...
- 制作约束：少大型派对和群像，用近景、文件、手机和门口对峙推进。 [MUST]
...
""",
    ...
    write(project / "90-内部工作稿" / "质检检查点.md", common_qc(project_name, "TikTok 欧美"))
...
- 欧美项目：`projects/dogfood-us-nda-heiress-60`
- 测试目标：模拟用户从立项、策划、分集大纲、分批正文、回退复检到交付整合的完整使用过程。

## 执行口径

- 两个项目都生成 60 集正式分集大纲和 60 个场次执行稿。
- 国内项目按 standard 交付；欧美项目按 production-enhanced 交付并预置增强包文件。
